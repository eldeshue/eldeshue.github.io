...
- [select](select.md) : FD들을 모니터링함, 이후 특정 FD가 가리키는 소켓의 상태 변화 감지
- [kqueue](kqueue.md) : select의 개선된 버젼, BSD 계열 특화 구현 -> **리눅스 환경 전환에 따라 [[epoll]] 사용.** 
- send : 데이터 전송, write로 대체 가능, flag로 보다 세밀한 옵션 설정 가능
- recv : 데이터 수신, read로 대체 가능, flag로 보다 세밀한 옵션 설정 가능
## sockaddr 구조체 관련
...
# Reference
-  irc 프로토콜 상세 : https://modern.ircdocs.horse/
-  irc 클라이언트 참고 : https://github.com/solanum-ircd/solanum